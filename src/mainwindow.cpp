    ...
    ui(new Ui::MainWindow)
{
    ...
    ui->setupUi(this);
    logger = new Logger(this, QDir(QStandardPaths::writableLocation(QStandardPaths::AppDataLocation)).filePath("quiver-lite-wallet.log"));
    ...
    QObject::connect(ui->actionFile_a_bug, &QAction::triggered, [=]() {
        QDesktopServices::openUrl(QUrl("https://github.com/Arrowchain/quiver-lite/issues/new"));
    });
...
void MainWindow::setupSettingsModal() {
    ...
    QObject::connect(ui->actionSettings, &QAction::triggered, [=]() {
        ...
        QObject::connect(settings.comboBoxTheme, &QComboBox::currentTextChanged, [=] (QString theme_name) {
            ...
            QMessageBox::information(this, tr("Restart"), tr("Please restart Quiver to have the theme apply"), QMessageBox::Ok);
        });
        ...
        settings.cmbServer->addItem("https://arrow.cryptoforge.cc:443");
...
void MainWindow::donate() {
    ...
    ui->MemoTxt1->setText(tr("Thanks for supporting Quiver!"));

    ui->statusBar->showMessage(tr("Donate 0.01 ") % Settings::getTokenName() % tr(" to support Quiver"));
...
void MainWindow::addNewZaddr(bool sapling) {
    rpc->createNewZaddr(sapling, [=] (json reply) {
        ...
        // Make sure the RPC class reloads the as-addrs for future use
        rpc->refreshAddresses();

        // Just double make sure the as-address is still checked
//        if ( sapling && ui->rdioZSAddr->isChecked() ) {
            ui->listReceiveAddresses->insertItem(0, addr);
            ui->listReceiveAddresses->setCurrentIndex(0);

            ui->statusBar->showMessage(QString::fromStdString("Created new asAddr") %
...
std::function<void(bool)> MainWindow::addZAddrsToComboList(bool sapling) {
    return [=] (bool checked) {
        if (checked) {
            ...
            // If as-addrs are empty, then create a new one.
            if (addrs.isEmpty()) {
                addNewZaddr(sapling);
            }
        }
...
//        }
    ...
    QObject::connect(ui->tabWidget, &QTabWidget::currentChanged, [=] (int tab) {
        if (tab == 2) {
            // Switched to receive tab, select the as-addr radio button
